...
let loadedLibAntimony: any;

/** Loads libantimony and assigns the loaded module to the global variable. */
const initLoad = async () => {
  //@ts-ignore - libantimony is loaded in index.html
  loadedLibAntimony = await libantimony();
};

/** Check libantimony has loaded and throw if not. */
const checkLoaded = () => {
  if (!loadedLibAntimony) {
...
const createBinding = <TReturn, TParams extends any[]>(
  ...
  return (...params: TParams): TReturn => {
    checkLoaded();
    if (!binding) {
      binding = loadedLibAntimony.cwrap(name, returnType, paramTypes);
    }
